...
<body class="bg-green-50 text-green-900 min-h-screen flex flex-col">
  ...
  <div class="flex flex-1 overflow-hidden">
    ...
    <!-- Main content -->
    <main class="flex-1 p-8 overflow-auto flex justify-center items-start">
      <div class="form-box bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        <form action="view-invoices.php" method="POST" class="flex flex-col">
          <h3 class="text-2xl font-semibold mb-6 text-green-800 text-center">Enter Your Agent Code</h3>
          <input 
            type="text" 
            name="agent_code" 
            id="input-text" 
            required 
            class="border border-green-300 rounded px-4 py-2 mb-6 focus:outline-none focus:ring-2 focus:ring-green-400 transition"
            placeholder="Agent Code"
          >
          <input 
            type="submit" 
            value="Submit" 
            class="bg-green-600 hover:bg-green-700 text-white rounded px-4 py-2 font-semibold cursor-pointer transition"
          >
        </form>
      </div>
    </main>
