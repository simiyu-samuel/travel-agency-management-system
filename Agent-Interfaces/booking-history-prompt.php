...
<head>
  <meta charset="UTF-8" />
  <title>Booking History | Enter Agent Code</title>
...
<body class="bg-green-50 min-h-screen font-sans flex flex-col">
  ...
  <div class="flex flex-1 min-h-0">
    ...
    <!-- Main Content -->
    <main class="flex-1 p-10 overflow-auto min-h-0 flex items-center justify-center">
      <form action="booking-history.php" method="post" class="bg-white p-8 rounded shadow-md w-full max-w-md">
        <h3 class="text-2xl font-semibold text-green-700 mb-6 text-center">Enter Your Agent Code</h3>
        <input 
          type="text" 
          name="agent_code" 
          id="agent_code" 
          ...
          class="w-full px-4 py-3 mb-6 border border-green-300 rounded focus:outline-none focus:ring-2 focus:ring-green-500"
        />
        <button 
          type="submit"
          class="w-full bg-green-700 text-white py-3 rounded font-semibold hover:bg-green-800 transition"
        >
          Submit
        </button>
      </form>
